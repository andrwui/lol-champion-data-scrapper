...
const scrapeChampion = async (page: any, champion: any) => {
  ...
  const info = await page.evaluate(() => {
    ...
    const description: string =
      document
        .querySelector('.portable-infobox .pi-item div span')
        .innerHTML.charAt(0)
        .toUpperCase() +
      document
        .querySelector('.portable-infobox .pi-item div span')
    ...
    const scrapeSidebar = (dataSource: string): string[] | string => {
      ...
      const parent: HTMLElement = document.querySelector(
        `.portable-infobox .pi-item[data-source=${dataSource}] .pi-data-value`,
      );
...
const main = async () => {
  ...
  const champions = await page.evaluate(() => {
    const championsElements: HTMLElement[] = Array.from(
      document.querySelectorAll('tr td span span a'),
    );
    const championsData = championsElements.map((champ: any) => ({
      url: champ.href,
    }));
  ...
  for (const champion of champions) {
    ...
    counter++;
    console.log({ id: counter, ...champ });
    scrapedData.push({ id: counter, ...champ });
  }
  ...
  fs.writeFile('champs.json', JSON.stringify(scrapedData), (err: Error) => {
    if (err) throw err;
